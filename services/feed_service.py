...
def get_friends_listening_now(user_id: str) -> list[dict]:
    ...
    # "Listening now" means today — cut off at the start of the current calendar
    # day (UTC), not a rolling 24-hour window, so last night's plays drop off at
    # midnight instead of lingering until the same time tomorrow.
